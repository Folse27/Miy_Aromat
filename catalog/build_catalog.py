...
TYPE_MAP = {
    ...
    # догляд / body care
    "парфюмированный дезодорант": "Парфумований дезодорант",
    "парфумований дезодорант": "Парфумований дезодорант",
    "парфюмированный гель": "Парфумований гель",
    "парфумований гель": "Парфумований гель",
    "парфюмированный лосьон": "Парфумований лосьйон",
    "парфумований лосьйон": "Парфумований лосьйон",
    "парфюмированный крем": "Парфумований крем",
    "парфюмированный шампунь": "Парфумований шампунь",
    "парфюмированный спрей": "Парфумований спрей",
    "спрей для тела": "Спрей для тіла",
}
SEASON_WORDS = [("зим", "зима"), ("весн", "весна"), ("весен", "весна"),
                ("літ", "літо"), ("летн", "літо"), ("осін", "осінь"), ("осен", "осінь")]
CARE_TYPES = {"Парфумований дезодорант","Парфумований гель","Парфумований лосьйон",
              "Парфумований крем","Парфумований шампунь","Парфумований спрей","Спрей для тіла"}
...
def norm_format(raw, name, vol):
    low = (name or "").lower()
    if "набір" in low or "набор" in low or re.search(r"\bset\b", low):
        return "Набір"
...
def main(feed_path, out_dir):
    ...
    for ev, el in ET.iterparse(feed_path, events=("end",)):
        ...
        try:
            ...
            ptype = TYPE_MAP.get(params.get("Тип", "").strip().lower(),
                                 clean_text(params.get("Тип", "")).replace("-", ""))
            pform = norm_format(params.get("Формат"), name, vol)
            if ptype in CARE_TYPES and pform not in ("Пробник", "Мініатюра", "Набір"):
                pform = "Догляд"          # не «Флакон»: це засіб догляду
    ...
    # newest first (KeepinCRM ids increase over time), in-stock before sold-out
    products.sort(key=lambda p: (p["qty"] <= 0, -_idnum(p)))

    # ---- lightweight payload: ALL products, no desc/photos ------------
    LIGHT = ("id","sku","brand","name","vol","type","form","price","qty","photo","notes","aroma","season")
    light = [{k: p[k] for k in LIGHT} for p in products]
    js = "window.PRODUCTS=" + json.dumps(light, ensure_ascii=False, separators=(",", ":")) + ";"
    with open(f"{out_dir}/products.js", "w", encoding="utf-8") as f:
        f.write(js)

    # ---- heavy details, sharded by id: loaded only when a popup opens --
    import os as _os
    dd = f"{out_dir}/details"
    _os.makedirs(dd, exist_ok=True)
    for old_f in _os.listdir(dd):
        if old_f.endswith(".js"): _os.remove(f"{dd}/{old_f}")

    SHARDS = 16
    buckets = [dict() for _ in range(SHARDS)]
    for p in products:
        if not p["desc"] and len(p.get("photos") or []) < 2:
            continue                       # нічого довантажувати
        try: n = int(p["id"]) % SHARDS
        except (ValueError, TypeError): n = 0
        buckets[n][p["id"]] = {"d": p["desc"], "p": p.get("photos") or []}

    shard_mb = 0
    for i, b in enumerate(buckets):
        body = ("window.DETAILS=window.DETAILS||{};Object.assign(window.DETAILS,"
                + json.dumps(b, ensure_ascii=False, separators=(",", ":")) + ");")
        with open(f"{dd}/d{i}.js", "w", encoding="utf-8") as f:
            f.write(body)
        shard_mb = max(shard_mb, len(body.encode()) / 1e6)

    # старий великий файл більше не потрібен
    legacy = f"{out_dir}/products-all.js"
    if _os.path.exists(legacy): _os.remove(legacy)

    instock = sum(1 for p in products if p["qty"] > 0)
    print(f"OK: {len(products)} products total ({instock} in stock), {skipped} skipped (deleted)")
    print(f"  notes translated: {notes_kept}, dropped (untranslatable): {notes_dropped}")
    print(f"  products.js (усі товари, лише список): {len(js.encode())/1e6:.1f} MB")
    print(f"  details/d0..d{SHARDS-1}.js: {SHARDS} shards, max {shard_mb:.1f} MB each")
